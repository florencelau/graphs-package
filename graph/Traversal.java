...
public abstract class Traversal {
     ...
     * Used the algorithm from Hilfinger's Lecture 34. */
    public void traverse(Collection<Integer> V0) {
        clear();
        _fringe.addAll(V0);
        while (!_fringe.isEmpty()) {
            int v = _fringe.remove();
            if (marked(v) && !postvisited.contains(v) && shouldPostVisit(v)) {
                postVisit(v);
                postvisited.add(v);
            } else if (!marked(v)) {
                mark(v);
    ...
    /** Returns the list of marked vertices. For the purposes of testing. */
    protected ArrayList testingMarked() {
        return _marked;
    }
